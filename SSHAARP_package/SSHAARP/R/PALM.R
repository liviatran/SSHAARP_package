...
PALM<-function(motif, filename=SSHAARP::solberg_dataset, color=TRUE, filterMigrant=TRUE){
  ...
  #place current working directory in wd variable
  wd<-getwd()
  ...
  #set wd to tempdir() to store outpit files
  setwd(tempdir())
  ...
  if(OS=="Windows"){
    ...
    shell("for /f 'usebackq' %C in (`type filename`) do DEL gmt.history upperbound decile.cpt deciles motif motif.block motif.grd motif.xyz %C.ps filename upper_decile upper_round")
  }
  ...
  ###############MAC SPECIFIC COMMAND LINE ARGUMENTS FOR MAP GENERATION###############
    ...
    #move final output of motif.jpg to working directory
    quiet(file.move(paste(tempdir(),"/", motif, ".jpg", sep=""), wd, overwrite = T))
  }
}
