#verifyAlleleDataset v 2.0.0 3JAN2021
...
#'
#'@examples
#'#Example of a valid allele that is present in IMGTprotalignments, but not present in the Solberg dataset with more than two fields.
#'verifyAlleleDataset("A*01:37:01", filename=solberg_dataset)
#'
#'#Example of a valid allele that is present in IMGTprotalignments, but not present in the Solberg dataset with two fields.
#'verifyAlleleDataset("A*01:36", filename=solberg_dataset)

verifyAlleleDataset<-function(allele, filename){
  ...
  alleleANHIGcheck<-verifyAlleleANHIG(allele, filename)
  ...
  if(nrow(filteredAllele) == 0 & length(endMess)!= 0){
   return(list(FALSE, append(endMess, paste("The", allele, "allele entered is a valid allele and exists in IMGTprotalignments, but is not present in the Solberg dataset."))))
  }

  if(nrow(filteredAllele) == 0){
    return(list(FALSE, paste("The", allele, "allele entered is a valid allele and exists in IMGTprotalignments, but is not present in the Solberg dataset.")))
  }
