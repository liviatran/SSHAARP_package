## Population Allele Locating Mapmaker v 2.0.0 18AUG2024
#'Population Allele Locating Mapmaker
#'
#'Produces a frequency heatmap for a specified allele or amino-acid motif, based on the allele frequency data in the Solberg dataset.
#'
...
#'@param mapScale A logical parameter that determines if the max value of the map scale should be 1 (FALSE), or if it should represent the maximum frequency of the chosen motif, allele, or haplotype (TRUE). The default value is TRUE.
...
#'@param generateLowFreq A logical parameter that determines whether maps should be generated for a variant if the maximum frequency for the variant is low frequency. Low frequency populations have 3 0's after the decimal point.
#'@param resolution An integer for raster resolution in dpi for the final map output. Default is set to 2000.
#'
...
#'\dontrun{PALM("DRB1*26F~28E~30Y", variantType="motif", mask = FALSE, filterMigrant=TRUE, filename = solberg_dataset)}
#'
#'#Example to produce an allele greyscale heat map where migrant populations are not filtered, mask is on
#'\dontrun{PALM("DRB1*01:01", variantType="allele", mask = TRUE, color=FALSE, filterMigrant=FALSE, filename = solberg_dataset)}
#'
...
#'\dontrun{PALM("DRB1*01:01~A*01:01", variantType = "haplotype", filename = mock_haplotype_dataset)}
#'
#'@references Solberg et.al. (2008) <doi: 10.1016/j.humimm.2008.05.001>
#'
#'

PALM<-function(variant, variantType, mask=FALSE, color=TRUE, filterMigrant = TRUE, mapScale = TRUE, direct= getwd(), filename, AFND=TRUE, generateLowFreq = TRUE, resolution = 2000){
  ...
  if(variantType == "haplotype"){
    ...
    dataset <- dataSubsetHaplo(variant, filename, AFND)
  ...
  if(variantType=="motif"){
    ...
    fMresults<- findMotif(variant, filename)
  ...
  if(variantType == "allele"){
    ...
    vAresults<-verifyAlleleDataset(variant, filename)
  ...
  if(OS=="Windows"){
    ...
    if(nchar(strsplit(as.character(decile_interval[2]), ".", fixed=T)[[1]][[2]]) >=5){
      ...
      invisible(shell("for /f 'usebackq' %C in (`type filename`) do gmt psscale -Dx0.1i/0.9i/1.5i/0.3i -Cdecile.cpt --FONT_ANNOT_PRIMARY=6.5p,Helvetica,black -Np -L -O -K >> %C.ps", intern=T))
    }

    if(nchar(strsplit(as.character(decile_interval[2]), ".", fixed=T)[[1]][[2]]) <5){
      #adds color scale to basemap based on cpt provided -- makes font on legend smaller
      invisible(shell("for /f 'usebackq' %C in (`type filename`) do gmt psscale -Dx0.1i/0.9i/1.5i/0.3i -Cdecile.cpt --FONT_ANNOT_PRIMARY=8p,Helvetica,black -Np -L -O -K >> %C.ps", intern=T))
    }
    ...
    if(nchar(strsplit(as.character(format(decile_interval[2], scientific = FALSE)), ".", fixed=T)[[1]][[2]]) >=5){
      gmt.system("gmt psscale -Dx0.1i/0.9i/1.5i/0.3i -Cdecile.cpt --FONT_ANNOT_PRIMARY=6.5p,Helvetica,black -Np -L -O -K >> `cat variant`.ps")
    } else{
      #adds color scale to basemap based on cpt provided -- makes font on legend smaller
      gmt.system("gmt psscale -Dx0.1i/0.9i/1.5i/0.3i -Cdecile.cpt --FONT_ANNOT_PRIMARY=8p,Helvetica,black -Np -L -O -K >> `cat variant`.ps")
    }
